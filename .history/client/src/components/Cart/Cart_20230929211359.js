...
function Cart() {
  ...
  const [cartItems, setCartItems] = useState([]); // similar to useSelector((state) => state.cart)

  const updateItemQuantity = (idx, newItemQuantity) => {
    setCartItems(prevCartItems => {
      return prevCartItems.map((item, itemIdx) =>
        itemIdx === idx ? { ...item, quantity: newItemQuantity } : item
      );
    });
  };
  useEffect(()=>{
              ...
              <hr style={{width:'100%', border:'2.87px solid #E1E1E1',}} />
                {
                  cartItemsFromRedux.length > 0?(
                    cartItemsFromRedux.map((item, idx)=>(
                      <div className={styles.details} key={idx}>
                        ...
                        <div style={{display:'flex', flexDirection:'column', width:'10%', textAlign:'start', fontWeight:'500'}}>
                          ...
                          <select className={styles.quantity} value={item.quantity || 1} onChange={(e) => updateItemQuantity(idx, Number(e.target.value))}>
                        ...
                        <div style={{display:'flex', flexDirection:'column', width:'20%', textAlign:'start', fontWeight:'500', paddingLeft:'3rem'}}>
                          ...
                          <span style={{marginBottom:'0.7rem'}}>₹{handleTotalPrice(item.price, item.id)}</span>
